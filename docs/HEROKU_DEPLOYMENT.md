...
- Heroku Key-Value Store's `REDIS_URL` is used automatically for rate limiting, including Heroku's documented self-signed TLS client setting. Set `RATELIMIT_STORAGE_URI` only to override it or when a third-party provider uses a different config-var name; explicit providers retain normal certificate verification. Limits are shared across workers, so tune `WEB_CONCURRENCY` for capacity rather than rate-limit consistency.
