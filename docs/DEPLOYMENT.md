...
This app is set up for Heroku-style deployment with a release migration step and production-only startup validation.
...
That means each deploy should run migrations before the web dyno starts serving traffic.
...
Application logs are emitted to stdout in structured JSON-friendly lines so they can be consumed by Heroku log drains and error monitoring.

## Remaining External Work

These steps still need to be performed in Heroku itself:

1. Provision separate staging and production apps, each with its own Heroku Postgres database.
2. Set config vars in the Heroku dashboard or CLI. Never put production values in `.env.example`, GitHub Actions, or committed files.
3. Enable Heroku Automatic Certificate Management for custom domains and verify all user traffic is HTTPS.
...
5. Schedule PGBackups, choose retention, and complete a restore drill into staging before accepting user data.
6. Attach a log drain and exception/alert monitoring, then point uptime checks at `/healthz` and `/readyz`.
7. Run `heroku run flask provision-admin --app <production-app> --username <name> --email <email>` while registration is disabled.
